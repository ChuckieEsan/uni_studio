...
def uploader():
    ...
    if file_suffix in ['png','PNG','jpg','JPG','JPEG','jpeg']:
        try:
            ...
            msg['success'] = 1;msg['data']='图片上传成功'
        except Exception as e:
            msg['data'] = repr(e)
    else:
        try:
            _file.save(path+'/audio.mp3')
            msg['success'] = 1;msg['data']='音频上传成功'
        except Exception as e:
            msg['data'] = repr(e)
    return jsonify(msg)
    ...
    if request.method=='POST':
        ...
        if request.headers.get('openid') and Users.query.filter(Users.wxid==request.headers.get('openid')).first():
            try:
                ...
                new_card = Cards(wx_openid,title,description,content,with_audio=with_audio,with_img=with_img,crop=crop,display=display,dir_name=dir_name)
                try:
                    ...
                    try:
                        ...
                        msg['data'] = repr(e);msg['errorCode']=5
                except Exception as e:
                    db.session.rollback()
                    msg['data'] = repr(e);msg['errorCode']=3
            except Exception as e:
                msg['data'] = repr(e) ;msg['errorCode']=4
        else:
        ...
        if request.values.get('id') and request.values.get('id').isdigit():
            ...
            if this_card:
                ...
                msg['data'] = this_card;msg['success']=1;msg['data']['is_liked']=False
                ...
                msg['data'] = 'not found';msg['errorCode'] = 10
        else:
            msg['data'] = 'invalid id';msg['errorCode'] = 11
                        ...
                        if json_data.get('likes'):
                            ...
                            if op_val == '1':
                                ...
                                if this_user.id not in json.loads(_card.liked_by):
                                    ...
                                    try:
                                        db.session.commit()
                                        msg['data']='点赞成功';msg['success']=1
                                    except Exception as e:
                                        db.session.rollback()
                                        msg['data'] = repr(e);msg['errorCode']=18
                                else:
                                    msg['data']='只能点赞一次';msg['errorCode']=23
                            elif op_val == '-1':
                                op_num = -1
                                result_likes = _card.likes + op_num
                                result_liked_by = json.loads(_card.liked_by)
                                if this_user.id in result_liked_by:
                                    ...
                                    try:
                                        db.session.commit()
                                        msg['data']='取消点赞成功';msg['success']=1
                                    except Exception as e:
                                        db.session.rollback()
                                        msg['data'] = repr(e);msg['errorCode']=18
                                else:
                                    msg['data']='未点赞，不能取消';msg['errorCode']=29
                            else:
                                msg['data']='invalid operand';msg['errorCode']=28
                        else:
                            ...
                            try:
                                ...
                                db.session.rollback()
                                msg['data']=repr(e);msg['errorCode']=24
                    else:
                        msg['data']='params missing';msg['errorCode']=19
                except Exception as e:
                    msg['data']=repr(e);msg['errorCode']=20
            else:
                msg['data']='unauthorized';msg['errorCode']=22
        else:
            msg['data']='unauthorized';msg['errorCode']=21
            ...
            if this_user:
                ...
                if this_card and str(this_card.wx_openid)==str(this_user.wxid):
                    try:
                        ...
                        db.session.commit()
                        msg['data']='删除成功';msg['success']=1
                    except Exception as e:
                        msg['data']=repr(e);msg['errorCode']=27
            else:
...
def pcode():
    ...
    if request.method=='GET':
        if request.headers.get('openid') and Users.query.filter(Users.wxid==request.headers.get('openid')).first() and request.values.get('id'):
            ...
            try:
                ...
                if id.isdigit():
                    ...
                    id = 0
                if not err_code:
                    r2 = requests.post('https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token='+access_token,data=json.dumps({'scene':str('id='+str(id))}),headers={
                        "Accept":"text/html,image/jpg,*/*;q=0.8"
                    })
                    if len(r2.text)>100:
                        try:
                            ...
                            msg['data'] = repr(e);msg['errorCode']=29
                    else:
                        result2 = json.loads(r2.text)
                        err_code = result2.get('errcode')
                        msg['data'] = result2.get('errmsg');msg['errorCode']=err_code
                else:
                    msg['data'] = err_msg;msg['errorCode']=err_code
            except Exception as e:
                msg['data'] = repr(e);msg['errorCode']=16
        else:
            msg['errorCode']=17;msg['data']='unauthorized'
        return jsonify(msg)
    data = request.get_data()
            ...
            if err_code==None:                    # 请求json2code正常的时候返回的err_code 是空   这里会报错
                if Users.query.filter(Users.wxid==wx_openid).first() == None:
                    ...
                    try:                            
                        ...
                        db.session.commit()
                        msg['new'] = 1;msg['data'] = {'openid':wx_openid};msg['success'] = 1
                    except Exception as e:
                        msg['data']=repr(e);msg['errorCode']=8
                else:
                    msg['data'] = {'openid':wx_openid};msg['success'] = 1
            else:
                msg['data']=err_msg;msg['errorCode'] = err_code
        except Exception as e:
            msg['data'] = repr(e);msg['errorCode']=6
    except Exception as e:
...
def user_handler():
    ...
    if request.headers.get('openid') and Users.query.filter(Users.wxid==request.headers.get('openid')).first():
        this_user = Users.query.filter(Users.wxid==request.headers.get('openid')).first()
        if request.method=='GET':
            user_cards = process_sqlalchemy_arr(Cards.query.filter(Cards.wx_openid==request.headers.get('openid')).order_by(Cards.create_time.desc()).all())
            ...
            msg['data'] = {'user_cards':user_cards,'user_saves':user_saves_list,'role':this_user.role_id};msg['success']=1      
            return jsonify(msg)
        elif request.method=='PUT':
            data = request.get_data()
            try:
                ...
                msg['data']=repr(e);msg['errorCode']=33
                return jsonify(msg)
            if new_save and json_data.get('id'):
                if Cards.query.filter(Cards.id==json_data.get('id')):
                    ...
                    if str(new_save)=='1' and json_data.get('id') not in old_card_list:
                        ...
                        try:
                            db.session.commit()
                            msg['success']=1;msg['data']='收藏成功'
                        except Exception as e:
                            msg['data']=repr(e);msg['errorCode']=31
                    elif str(new_save)=='-1' and json_data.get('id') in old_card_list:
                        old_card_list.remove(json_data.get('id'))
                        base_user_query.update({'cards_saved':json.dumps(old_card_list)})
                        try:
                            db.session.commit()
                            msg['success']=1;msg['data']='取消成功'
                        except Exception as e:
                            msg['data']=repr(e);msg['errorCode']=32
                    else:
                        msg['data']='operand error';msg['errorCode']=30
                else:
                    msg['data']='operand error';msg['errorCode']=34
            else:
                msg['data']='operand error';msg['errorCode']=35
            ...
            if new_role_id and str(new_role_id).isdigit() and Roles.query.filter(Roles.id==new_role_id).first():
                ...
                try:
                    db.session.commit()
                    msg['success']=1;msg['data']='更新成功'
                except Exception as e:
                    db.session.rollback()
                    msg['data']=repr(e);msg['errorCode']=15
            return jsonify(msg)
    else:
        return do_return(403,'unauthorized')  
...
def template_handler():
    ...
    if request.method=='GET':
        ...
        msg['success']=1;msg['data']=templates
