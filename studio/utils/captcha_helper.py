...
def getcaptcha(len: int = 6) -> str:
    return ''.join(
        random.sample([
            '2', '3', '4', '5', '6', '7', '8', '9', 'z', 'y', 'x', 'w', 't', 's', 'r', 'q', 'p', 'n', 'm', 'k', 'j',
            'i', 'h', 'g', 'f', 'e', 'd', 'c', 'b', 'a'
        ], len))
...
def img_to_base64():
    ...
    # 创建Font对象:
    font = ImageFont.truetype(os.path.join(os.getcwd(), 'studio', 'utils', 'Arial.ttf'), 36)
    # 创建Draw对象:
...
def _get_captcha_img(cpt: str) -> bytes:
    width = 60 * len(cpt)
    height = 60
    #  生成图片
    image = Image.new('RGB', (width, height), (255, 255, 255))
    ...
    byte_data = out_buffer.getvalue()
    base64_str = base64.b64encode(byte_data)
    return str(b'data:image/jpeg;base64,' + base64_str, encoding='utf-8')
