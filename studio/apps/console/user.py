...
def role_add():
    role_max = db.session.query(
        func.max(UserRoles.role_bit).label('role_bit')).one()
    i = 0
    if role_max.role_bit:
        while 1 << i < role_max.role_bit:
            i = i+1
    role_bit = 1 << (i+1)
    _u = UserRoles(role_bit=role_bit, role_text=request.values.get(
        'role_text'), description=request.values.get('description'))
    db.session.add(_u)
    db.session.commit()
