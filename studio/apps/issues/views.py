...
def root():
    if request.method == 'GET':
        ...
        issuetypes = IssueTypes.query.order_by(
            IssueTypes.priority.desc()).all()
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        issueType = IssueTypes.query.filter(
            IssueTypes.typevalue == request.form.get('type')).first()

        to = UserUsers.query.filter(
            UserUsers.id == issueType.duty_user).first().email
        try:
            send_mail(to=to, content=str(issue),
                      subject='dutbit.com 新增反馈通知', type='plain')
            flash('发送成功')
        ...
        if g.user:
            to = UserUsers.query.filter(
                UserUsers.role_bits.op('&')(1) == 1).first()
            head = start_chat(g.user.id, to.id,
                              first_alias=g.user.email, second_alias='反馈处理')
            send_chat(from_id=g.user.id, text='查看反馈工单{}'.format(
                issue.id), to_id=to.id, head=head)
        flash('提交成功，感谢您的反馈')
        return redirect(url_for('issues.root', timeout=True))
